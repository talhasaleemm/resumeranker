...
- **Messy/Real-World Resumes (Degraded Output)**: While parsing tests now include real-world formats, extraction on multi-column PDFs (via PyMuPDF) interleaves text horizontally across columns, failing to cleanly isolate structured fields like "skills". Table-based DOCX files perform reasonably well, but multi-column PDFs produce corrupted structured output.
